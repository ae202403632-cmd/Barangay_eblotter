            ...
            <?php if ($result && $result->num_rows > 0): ?>
                <?php $i=1; while($row=$result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($row['resolution_text']) ?></td>
                        <td><?= htmlspecialchars($row['resolution_date']) ?></td>
                        <td>
                            <a href="edit.php?id=<?= $row['resolution_id'] ?>" class="link-edit">Edit</a>
                            <a href="delete.php?id=<?= $row['resolution_id'] ?>"
                               class="link-delete"
                               onclick="return confirm('Delete this resolution?');">
                                Delete
                            </a>
                        </td>
                    </tr>
                <?php endwhile; ?>
            <?php else: ?>
                <tr><td colspan="6" class="text-center text-muted">No resolutions found.</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
